...
class AppStyle {
  AppStyle({Size? screenSize}) {
    ...
    const int tabletSm = 600;
    const int phoneLg = 400;
...
class _Text {
  ...
  final TextStyle _monoTitleFonts = const TextStyle(fontFamily: 'B612Mono');

  final TextStyle _quoteFonts = const TextStyle(fontFamily: 'Cinzel');

  final TextStyle _wonderTitleFonts = const TextStyle(fontFamily: 'Yeseva');

  final TextStyle _contentFonts =
      const TextStyle(fontFamily: 'Raleway', fontFeatures: [
    FontFeature.enable('kern'),
  ]);

  TextStyle _getFontForLocale(TextStyle fonts) {
    return fonts;
  }
